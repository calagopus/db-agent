use self::{mysql::MysqlExplorer, postgres::PostgresExplorer};
...
type TenantPoolValue = (std::time::Instant, TenantPool);
type TenantPoolMap = HashMap<(uuid::Uuid, String), TenantPoolValue>;
static TENANT_POOLS: LazyLock<Arc<tokio::sync::Mutex<TenantPoolMap>>> = LazyLock::new(|| {
    let pools = Arc::new(tokio::sync::Mutex::new(TenantPoolMap::new()));
...
pub async fn close_pools(instance: uuid::Uuid) {
    let mut evicted = Vec::new();
    let mut pools = TENANT_POOLS.lock().await;
    pools.retain(|(uuid, _), (_, pool)| {
        if *uuid == instance {
            evicted.push(pool.clone());
            false
        } else {
...
fn unsupported_engine() -> anyhow::Error {
    crate::response::DisplayError::new("exploring this database type is not supported")
...
impl super::Instance {
    async fn connect_explorer(
        ...
        let pool = {
            let mut pools = TENANT_POOLS.lock().await;
            pools.get_mut(&key).map(|(last_used, pool)| {
        ...
        let pool = match pool {
            ...
            None => {
                ...
                let pool = match self.data.read().await.database_type {
                    DatabaseType::Mariadb => {
                        TenantPool::Mysql(MysqlExplorer::create_pool(&socket, database))
                    }
                    DatabaseType::Postgres => {
                        TenantPool::Postgres(PostgresExplorer::create_pool(&socket, database))
                    }
                    DatabaseType::Mongodb | DatabaseType::Redis => {
                        return Err(unsupported_engine());
                    }
                };
                ...
                TENANT_POOLS
                    .lock()
                    .await
                    .insert(key, (std::time::Instant::now(), pool.clone()));
        ...
        let mut inner: Box<dyn ExplorerConnection> = match pool {
            TenantPool::Mysql(pool) => Box::new(MysqlExplorer {
                connection: pool.acquire().await?,
                database: database.to_owned(),
            }),
            TenantPool::Postgres(pool) => Box::new(PostgresExplorer {
                connection: pool.acquire().await?,
            }),
        };
    ...
    pub async fn run_explorer_query(
        &self,
        database: &str,
        sql: &str,
        max_rows: u32,
...
fn like_pattern(operator: FilterOperator, value: &str) -> String {
    ...
    // only the three LIKE operators reach here, the catch-all stands in for Contains because
    // clippy::unreachable is denied
    match operator {
...
impl super::Instance {
    pub async fn browse_explorer_rows(
        &self,
        database: &str,
...
fn check_keys(columns: &[SchemaColumn], keys: &[RowValue]) -> Result<(), anyhow::Error> {
    ...
    if primary.len() != keys.len()
        || !primary
            .iter()
            .all(|name| keys.iter().any(|key| key.column == *name))
    {
...
impl super::Instance {
    pub async fn mutate_explorer_rows(
        &self,
        database: &str,
        schema: Option<&str>,
        table: &str,
...
impl super::Instance {
    pub async fn explorer_column_types(
        &self,
        database: &str,
    ) -> Result<Vec<String>, anyhow::Error> {
        match self.data.read().await.database_type {
            DatabaseType::Mariadb => {
                return Ok(mysql::MYSQL_TYPES.iter().map(ToString::to_string).collect());
            }
            DatabaseType::Postgres => {}
            DatabaseType::Mongodb | DatabaseType::Redis => return Err(unsupported_engine()),
        }
    ...
    pub async fn create_explorer_table(
        &self,
        database: &str,
        schema: Option<&str>,
        table: &str,
    ...
    pub async fn rename_explorer_table(
        &self,
        database: &str,
        schema: Option<&str>,
        table: &str,
    ...
    pub async fn drop_explorer_table(
        &self,
        database: &str,
        schema: Option<&str>,
        table: &str,
    ...
    pub async fn rename_explorer_column(
        &self,
        database: &str,
        schema: Option<&str>,
        table: &str,
    ...
    pub async fn drop_explorer_column(
        &self,
        database: &str,
        schema: Option<&str>,
        table: &str,
    ...
    pub async fn add_explorer_column(
        &self,
        database: &str,
        schema: Option<&str>,
        table: &str,
...
/// explorer-connection permit is released
async fn run_ddl(
