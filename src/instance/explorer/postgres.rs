...
impl PostgresExplorer {
    pub(super) fn create_pool(socket: &Path, database: &str) -> sqlx::Pool<sqlx::Postgres> {
        sqlx::pool::PoolOptions::new()
            ...
            .connect_lazy_with(super::super::connection::postgres::connect_options(
                socket, database,
            ))
    }
}
...
fn postgres_column(row: &PgRow) -> Result<SchemaColumn, sqlx::Error> {
    let cast_type: Option<String> = row.try_get("cast_type")?;
...
impl ExplorerConnection for PostgresExplorer {
    ...
    async fn schema_tables(&mut self) -> Result<DatabaseSchema, anyhow::Error> {
        ...
        let mut index: HashMap<(String, String), usize> = HashMap::with_capacity(tables.len());
