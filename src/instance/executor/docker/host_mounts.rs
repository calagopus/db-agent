...
pub struct HostMountTable {
    container_id: String,
    /// (destination inside the db-agent container, source on the engine host)
    ...
    pub fn validate_directories(
        ...
        for (directory, name) in directories {
            if self.translate(Path::new(directory)).is_none() {
                return Err(anyhow::anyhow!(
                    "{name} '{directory}' is not covered by any mount of the db-agent container, so the container engine on the host cannot see it. add a volume for it (e.g. '/path/on/host:{directory}')"
                ));
            }
        }
