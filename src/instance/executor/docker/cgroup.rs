    ...
    pub fn write_burst(&self, pid: i64, multiple: f64) -> BurstOutcome {
        ...
        match std::fs::write(&self.burst, burst_us.to_string()) {
                ...
                ) =>
            {
                if WARNED_UNSUPPORTED.set(()).is_ok() {
                    tracing::warn!(
                        "cfs burst cannot be written on this host (is db-agent running in a restricted container?), {}: {}",
                        self.burst.display(),
                        err
                    );
                }
...
impl StatsSampler {
    pub fn register(&self, files: StatFiles) -> SampleReceiver {
        ...
        self.thread.get_or_init(|| {
            ...
            std::thread::spawn(move || Self::run(&aborted, &incoming));
        });

        receiver
    }

    fn run(aborted: &AtomicBool, incoming: &parking_lot::Mutex<Vec<SamplerEntry>>) {
